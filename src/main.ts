...
document.querySelector(".ui")!.insertAdjacentHTML(
  "beforeend",
  `<button class="home-button" id="homeButton">HOME SCREEN</button>`,
);
document.querySelector(".ui")!.insertAdjacentHTML(
  "beforeend",
...
startFarmButton.addEventListener("click", () =>
  beginFarm(`Welcome, ${farmerNames[selectedFarmer]}. Your new farm is ready.`),
);
...
function returnToHomeScreen() {
  if (!homeArmed) {
    homeArmed = true;
    homeButton.textContent = "CONFIRM HOME";
    toast.textContent = "Save first to keep this farm, then press Home Screen again.";
    setTimeout(() => {
      homeArmed = false;
  ...
  // Reloading restores the initial character-selection screen and prevents a
  // half-finished running session from being mistaken for a new farm.
  window.location.reload();
}
